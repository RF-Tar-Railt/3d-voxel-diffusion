...
model.load_state_dict(torch.load('models/voxel_diffusion_only_mask.pth'))
model.eval()

diff = Diffusion(device)

samples = diff.ddim_sample(model, batch_size=9, channels=1)
print(samples.shape)
samples = samples.permute(0, 2, 3, 4, 1)
voxels = (((samples[..., 0] + 1) * 0.5) > 0.95).cpu().numpy()
